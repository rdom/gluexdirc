...
GlxCherenkovProcess::GlxCherenkovProcess(const G4String& processName, G4ProcessType type)
  : G4Cerenkov(processName, type)
{
 
}

G4VParticleChange* GlxCherenkovProcess::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
...

{
	...
	for (G4int i = 0; i < NumPhotons; i++) {
		...
		do {
			...
			if(cosTheta>1) {
			  std::cout<<"Warning - GlxCherenkovProcess:  cosTheta "<< cosTheta <<std::endl;
			   return pParticleChange;
			}
		...
		G4double phi = twopi*rand; //twopi*GlxManager::Instance()->GetTest()/360.;//twopi*rand;
		...
		aCerenkovPhoton->SetKineticEnergy(sampledEnergy);
                ...
                do {
		  rand = G4UniformRand();//GlxManager::Instance()->GetTest();//G4UniformRand();
...
G4double GlxCherenkovProcess::GetAverageNumberOfPhotons(const G4double charge,
                              const G4double beta, 
