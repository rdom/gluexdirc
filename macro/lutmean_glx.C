void lutmean_glx(TString baseFile = "../data/lut.root", Int_t b=0){
  ...
  fLutNew = new TClonesArray("GlxLutNode");
  ...
  for (Int_t inode=0; inode<fLut->GetEntriesFast(); inode++){
	...
	// hTime->Fill(tArray[j][v]);
      ...
      ((GlxLutNode*)(fLutNew->At(inode)))->AddEntry(node->GetDetectorId(), sum,j,node->GetNRefl(0),sumt, node->GetDigiPos(),node->GetDigiPos(),vArray[j].size()/(Double_t)size); 
    }
